...
int main() {
    // Create a vector of persons for the initial department
    vector<person> pliroforiki;
    pliroforiki.push_back(new person("Alice", "Smith"));
    pliroforiki.push_back(new person("Bob", "Johnson"));

    // Create a Secretary with the initial department
    Secretary s1(pliroforiki);

    // Create a new person
    person p1 = new person("panos", "anas");

    // Add the new person to the Secretary using the + operator
    s1 = s1 + p1;

    // View the updated list
    viewlist(cout, s1);

    // Don't forget to delete the dynamically allocated person object
    delete p1;
